...
import student.objdet_detect_from_cam as det_camera
...
##Load object detecton camera
model_camera = det_camera.create_model()
...
exec_detection = ['detctions_camera_model'] #'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
while True:
    try:
        ...
        # **Detctions from Object Detction Model**
        # Store measurments in z1
        if 'detctions_camera_model' in exec_list:
            camera1 = waymo_utils.get(frame.images, camera_name)
            image = waymo_utils.decode_image(camera1)
            detections = det_camera.detect(model_camera,image)
            z1 = []
            for (detect,score) in zip(detections["detection_boxes"],detections["detection_scores"]) :
                if score>0.8:
                    detection = detect
                    width = detection[3] - detection[1]
                    height = detection[2] - detection[0]
                    x_centre = detection[1] + width/2
                    y_centre = detection[0] + height/2
        ...
        if 'perform_tracking' in exec_list:
            ...
            # Using detections from our model
            meas_list_cam = []
            for meas in z1:                           
                meas_list_cam = camera.generate_measurement(cnt_frame, meas, meas_list_cam)
            ...
            if 'show_tracks' in exec_list:
                ...
                if 'make_tracking_movie' or True in exec_list:
